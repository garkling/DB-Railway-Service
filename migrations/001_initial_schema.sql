...
CREATE TABLE IF NOT EXISTS route (
    ...
    total_distance_km         DECIMAL(8, 2)          DEFAULT NULL COMMENT 'Total distance in km',
...
CREATE TABLE IF NOT EXISTS train (
    train_number        VARCHAR(10) NOT NULL COMMENT 'PK part 1',
    service_date        DATE        NOT NULL COMMENT 'PK part 2',
    scheduled_departure DATETIME    NOT NULL,
    scheduled_arrival   DATETIME    NOT NULL,
    duration_minutes    INT GENERATED ALWAYS AS
        (TIMESTAMPDIFF(MINUTE, scheduled_departure, scheduled_arrival)) STORED
        COMMENT 'Derived: arrival - departure in minutes',
    train_status        ENUM('on_time','delayed','cancelled') NOT NULL DEFAULT 'on_time',
    operator_code       VARCHAR(10) NOT NULL COMMENT 'FK to railway_operator',
    route_id            INT UNSIGNED NOT NULL COMMENT 'FK to route',
    PRIMARY KEY (train_number, service_date),
    CONSTRAINT fk_train_operator
        FOREIGN KEY (operator_code) REFERENCES railway_operator (operator_code)
        ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_train_route
        FOREIGN KEY (route_id) REFERENCES route (route_id)
        ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Train service entity type';
...
CREATE TABLE IF NOT EXISTS coach (
    ...
    coach_number         SMALLINT UNSIGNED NOT NULL COMMENT 'Coach number within train',
    coach_type           VARCHAR(30)       NOT NULL COMMENT 'Seating | Sleeping | Dining | Luggage',
    seat_capacity        SMALLINT UNSIGNED NOT NULL DEFAULT 0,
    has_air_conditioning BOOLEAN           NOT NULL DEFAULT TRUE,
    class_code           VARCHAR(5)        NOT NULL COMMENT 'FK to travel_class',
    owner_operator_code  VARCHAR(10)                DEFAULT NULL COMMENT 'FK to railway_operator (optional)',
    PRIMARY KEY (train_number, service_date, coach_number),
    CONSTRAINT fk_coach_train
        FOREIGN KEY (train_number, service_date) REFERENCES train (train_number, service_date)
...
CREATE TABLE IF NOT EXISTS ticket (
    train_number  VARCHAR(10)    NOT NULL COMMENT 'FK to train (part of PK)',
    service_date  DATE           NOT NULL COMMENT 'FK to train (part of PK)',
    ticket_number INT UNSIGNED   NOT NULL COMMENT 'Ticket number within train',
    seat_number   VARCHAR(10)             DEFAULT NULL,
    price_paid    DECIMAL(10, 2) NOT NULL,
    booking_date  DATE           NOT NULL,
    passenger_id  VARCHAR(20)    NOT NULL COMMENT 'FK to passenger',
    class_code    VARCHAR(5)     NOT NULL COMMENT 'FK to travel_class',
    ...
    CONSTRAINT fk_ticket_class
        FOREIGN KEY (class_code) REFERENCES travel_class (class_code)
        ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Ticket entity type (weak ET, identifying rel to Train)';
...
CREATE TABLE IF NOT EXISTS route_station (
    ...
    CONSTRAINT fk_rs_station
        ...
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Junction: Route connects Station (M:N)';

CREATE TABLE IF NOT EXISTS class_facility (
    class_code  VARCHAR(5)   NOT NULL,
    facility_id INT UNSIGNED NOT NULL,
    PRIMARY KEY (class_code, facility_id),
    CONSTRAINT fk_cf_class
        FOREIGN KEY (class_code) REFERENCES travel_class (class_code)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_cf_facility
        FOREIGN KEY (facility_id) REFERENCES facility (facility_id)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Junction: TravelClass provides Facility (M:N)';

CREATE TABLE IF NOT EXISTS operator_class_route_pricing (
    ...
    CONSTRAINT fk_pricing_operator
        FOREIGN KEY (operator_code) REFERENCES railway_operator (operator_code)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_pricing_class
        FOREIGN KEY (class_code) REFERENCES travel_class (class_code)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_pricing_route
        FOREIGN KEY (route_id) REFERENCES route (route_id)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Ternary: Operator charges for Class on Route';
...
CREATE TABLE IF NOT EXISTS first_class (
    ...
    CONSTRAINT fk_first_class_tc
        FOREIGN KEY (class_code) REFERENCES travel_class (class_code)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Specialization: FirstClass (class_code=1ST)';

CREATE TABLE IF NOT EXISTS second_class (
    class_code        VARCHAR(5) NOT NULL PRIMARY KEY,
    has_folding_table BOOLEAN    NOT NULL DEFAULT TRUE,
    CONSTRAINT fk_second_class_tc
        FOREIGN KEY (class_code) REFERENCES travel_class (class_code)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Specialization: SecondClass (class_code=2ND)';

CREATE TABLE IF NOT EXISTS sleeping_class (
    class_code           VARCHAR(5) NOT NULL PRIMARY KEY,
    ...
    CONSTRAINT fk_sleeping_class_tc
        FOREIGN KEY (class_code) REFERENCES travel_class (class_code)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Specialization: SleepingClass (class_code=SLP)';

CREATE TABLE IF NOT EXISTS seating_coach (
    train_number     VARCHAR(10)       NOT NULL,
    service_date     DATE              NOT NULL,
    coach_number     SMALLINT UNSIGNED NOT NULL,
    seat_arrangement VARCHAR(10)       NOT NULL DEFAULT '2+2' COMMENT 'e.g. 2+2, 2+1',
    PRIMARY KEY (train_number, service_date, coach_number),
    CONSTRAINT fk_seating_coach
        FOREIGN KEY (train_number, service_date, coach_number)
        REFERENCES coach (train_number, service_date, coach_number)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Specialization: SeatingCoach';

CREATE TABLE IF NOT EXISTS sleeping_coach (
    train_number           VARCHAR(10)       NOT NULL,
    service_date           DATE              NOT NULL,
    coach_number           SMALLINT UNSIGNED NOT NULL,
    ...
    PRIMARY KEY (train_number, service_date, coach_number),
    CONSTRAINT fk_sleeping_coach
        FOREIGN KEY (train_number, service_date, coach_number)
        REFERENCES coach (train_number, service_date, coach_number)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Specialization: SleepingCoach';

CREATE TABLE IF NOT EXISTS dining_coach (
    train_number     VARCHAR(10)       NOT NULL,
    service_date     DATE              NOT NULL,
    coach_number     SMALLINT UNSIGNED NOT NULL,
    ...
    PRIMARY KEY (train_number, service_date, coach_number),
    CONSTRAINT fk_dining_coach
        FOREIGN KEY (train_number, service_date, coach_number)
        REFERENCES coach (train_number, service_date, coach_number)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB COMMENT='Specialization: DiningCoach';

CREATE TABLE IF NOT EXISTS luggage_van (
    train_number     VARCHAR(10)       NOT NULL,
    service_date     DATE              NOT NULL,
    coach_number     SMALLINT UNSIGNED NOT NULL,
